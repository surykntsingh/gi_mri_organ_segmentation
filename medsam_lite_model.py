...
class MedSamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prompt_tensor = torch.tensor([[0, 0, self.img_shape[0]-1, self.img_shape[1]-1]]).float()
        if self.predict:
            return img_tensor, prompt_tensor
        else:
            ...
            return img_tensor, segment_tensor, prompt_tensor
...
def load_ite_medsam(medsam_lite_checkpoint_path):
...
class MedSamModel(pl.LightningModule):
    ...
    def __init__(self,ckpt_path, lr=1e-4, threshold=0.5):
        super().__init__()
        self.model = load_ite_medsam(ckpt_path)
    ...
    # @torch.no_grad()
    def predict_with_bbox_batch(self,images, bboxes):
        """
        ...
        pred_masks_probs = torch.sigmoid(low_res_logits)
        pred_masks_probs = torch.nn.functional.interpolate(
            pred_masks_probs,
            size=images.shape[-2:],
            mode="bilinear",
            align_corners=False
        )
        # pred_masks = (pred_masks_probs > 0.5).float()
    
        return pred_masks_probs  # (B, 1, H, W)

    def forward(self, image, prompt):
        pred_masks_probs = self.predict_with_bbox_batch(image, prompt)
        return pred_masks_probs

    def model_step(self, batch):
        images, masks, prompts = batch
        logits = self(images, prompts)
        probs = torch.sigmoid(logits)
        loss = 0.5 * self.bce(logits, masks) + 0.5 * self.dice_loss(logits, masks)
    ...
    def on_test_epoch_end(self):
        mean_hausdorff = np.mean(self.test_hausdorff)
        self.log("test_hausdorff", mean_hausdorff, prog_bar=True)
...
class MedsamDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ds = MedSamDataset(self.data_df, img_shape=self.image_shape, segment_type=self.segment_type)
...
class MedsamModelTrainer():
    ...
    def __init__(self, data_df, batch_size, dir_path,img_shape,segment_type,
        ...
        self.split_frac = split_frac
        model_ckpt_path = 'checkpoints/lite_medsam.pth'
        self.model = MedSamModel(model_ckpt_path)
        self.datamodule = MedsamDataModule(data_df, img_shape, segment_type, split_frac)
    ...
    def train(self, fast_dev_run=False):
        ...
        early_stop_callback = EarlyStopping(monitor= "val_loss", min_delta=0.001, patience = 2, verbose=True, mode="min")
...
if __name__=='__main__':
    ...
    dir_path = f'{MODEL_PATH}/models/medsam_lite/1'
    max_epochs = 20

    t = MedsamModelTrainer(data_meta_df, batch_size, dir_path, 
                                   image_shape, 'stomach_segmentation', 
                                   max_epochs=max_epochs)

    # t.train(fast_dev_run=True)
    # Define LoRA configuration
    lora_config = LoraConfig(
        task_type=TaskType.FEATURE_EXTRACTION,  # or TaskType.SEQ_CLS depending
    ...
    medsam_model = get_peft_model(t.model, lora_config)

    # Print trainable params
    medsam_model.print_trainable_parameters()
