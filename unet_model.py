...
class UNetModel(pl.LightningModule):
    ...
    def model_step(self, batch):
        ...
        loss = 0.5 * self.bce(logits, y) + 0.5 * self.dice_loss(probs, y)
...
class UnetModelTrainer():
    ...
    def train(self, fast_dev_run=False):
        ...
        # run test dataset
        test_metrics = self.trainer.test(self.model, datamodule = self.datamodule, verbose=True)
        pprint(test_metrics)
        metrics = {
            'train':train_metrics,
            'test':test_metrics
    ...
    def evaluate_visual(self, model_ckpt_path, threshold=0.5):
        self.model = UNetModel.load_from_checkpoint(model_ckpt_path)
        # self.model = self.model.load_from_checkpoint(self.checkpoint_path)
        self.datamodule.setup()
        batch = next(iter(self.datamodule.test_dataloader()))
        with torch.no_grad():
            self.model.eval()
            logits = self.model(batch[0].to('cuda'))
        ...
        for image, gt_mask, pr_mask in zip(batch[0], batch[1], pr_masks):
            ...
            _pr_mask=pr_mask.squeeze().detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    batch_size = 128
    image_shape = (128,128)
    dir_path = f'{MODEL_PATH}/models/unet/mgpu/0'
    max_epochs = 50
    model_trainers = {
